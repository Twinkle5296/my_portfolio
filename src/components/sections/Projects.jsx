...
const Projects = () => {
  ...
  // Category icons mapping (with fallback)
  const categoryIcons = {
    All: Target,
    'Web Apps': Globe,
  ...
  return (
    <section id="projects" className="relative py-20 bg-black overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute top-1/2 right-1/3 w-96 h-96 bg-primary/10 opacity-20 rounded-full" />
      </div>
          ...
          <div className="relative">
            ...
            {filteredProjects.length > 3 && (
              <div className="flex items-center justify-center gap-2 mt-8">
                {Array.from({
                  length: Math.max(0, filteredProjects.length - 3 + 1),
                }).map((_, index) => (
                  <button
                    key={index}
